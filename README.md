...
```bash
scripts/make-node-package.sh 0.20.1
```
...
```bash
tar -xJf ieum-chain_node_ubuntu_x86_64_v0.20.1.tar.xz
cd ieum-chain-node-v0.20.1
sudo ./install.sh
```
...
scripts/deploy-node-package.sh \
  download/ieum-chain_node_ubuntu_x86_64_v0.20.1.tar.xz \
  dev@192.168.1.148
```
