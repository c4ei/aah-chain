...
현재 버전은 `0.9.1`이며, locked/valid value, 단계별 합의 timeout,
이중투표 증거 저장·전파와 실제 4프로세스 네트워크 시험을 추가한
폐쇄형 테스트넷 단계입니다. 자세한 내용은
`docs/end/20260728_v0.0.9.1_BFT_SAFETY_NETWORK.md`를 참고하세요.
...
현재 버전은 `0.9.1`이며, 자동 부트스트랩 연결, 4노드 BFT 확정, 인증서 동기화,
locked/valid value와 이중투표 증거 영속화, 실제 동기화/피어 운영 상태 조회를 지원합니다.
