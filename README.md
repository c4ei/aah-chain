# IEUM Chain v0.20.7 쉬운 실행 안내

설치, systemd, 백업, 자동 업데이트 등 전체 운영 방법은
[`docs/USER_MANUAL_0.19.9.md`](docs/USER_MANUAL_0.19.9.md)에 정리되어 있으며,
v0.20.7 자동 복구·검증자 잔액 이전·P2P 주소 안정화 내용은
[`docs/VERSION_0.20.7.md`](docs/VERSION_0.20.7.md)를 참고하세요.
...
```bash
scripts/make-node-package.sh 0.20.7
```
...
```bash
tar -xJf ieum-chain_node_ubuntu_x86_64_v0.20.7.tar.xz
cd ieum-chain-node-v0.20.7
sudo ./install.sh
```
...
scripts/deploy-node-package.sh \
  download/ieum-chain_node_ubuntu_x86_64_v0.20.7.tar.xz \
  dev@192.168.1.148
```
